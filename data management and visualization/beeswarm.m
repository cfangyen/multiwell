...
if isfinite(p.Results.dot_size)
    ...
    for i=1:length(ux)
        ...
        switch lower(p.Results.overlay_style)
            ...
            case 'ci'
                line([1 1]*ux(i),mean(yorig(ic==i))+[-1 1]*std(yorig(ic==i))/sqrt(sum(ic==i))*tinv(0.975,sum(ic==i)-1),'Color',cmap(i,:),'LineWidth',1)
                line([ux(i)-2*rwid ux(i)+2*rwid],[1 1]*mean(yorig(ic==i)),'LineWidth',1,'Color',cmap(i,:))
        end
