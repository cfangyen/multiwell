...
def train_model(dataset_root, model_save_dir, model = None, num_classes=1, dataset_test_root=None, dataset_split=0.3, train_batch_size=2, test_batch_size=1, num_epochs=10, backbone='resnet50', dataset_class_params=None):
    ...
    data_loader = torch.utils.data.DataLoader(
        dataset, batch_size=train_batch_size, shuffle=True, num_workers=8,
        collate_fn=U.collate_fn)

    data_loader_test = torch.utils.data.DataLoader(
        dataset_test, batch_size=test_batch_size, shuffle=False, num_workers=8,
        collate_fn=U.collate_fn)
    ...
    for epoch in range(num_epochs):
        ...
        bbox_acc = bbox_eval_results[1]
        bbox_rec = bbox_eval_results[8]
        segm_acc = segm_eval_results[1]
    ...
    print("###################### Training done! ######################")
    training_data.close()
    model_save_dir = r'C:\Users\fffei\Dropbox\Paper\Egg-laying\Data\WormEggDetectModel'
...
if __name__ == "__main__":
    ...
    ##### Training
    dataset_root = r'C:\Users\fffei\Dropbox\Paper\Egg-laying\Data\WormTrainingData_2023July'
    dataset_test_root = None

    # Set up filepaths to save checkpoints after every epoch
    model_save_dir = r'C:\Users\fffei\Dropbox\Paper\Egg-laying\Data\WormTrainingData_2023July\TrainedModel'

    class_params = U.get_labeling_params(exclude=["L1-Larva", "L2-Larva", "L3-Larva", "L4-Herm", "L4-Male", "Adult-Herm", "Adult-Herm", "Adult-Male", "Dauer", "Unknown-Stage"]) # We can modify the label parameters before passing them to training if we want.
    ...
    model_dir = r'C:\Users\fffei\Dropbox\Paper\Egg-laying\Data\WormTrainingData_2023July\TrainedModel\model_2023-08-01_08-31-05_resnet50_2classes_25epochs_24Done.pth'
    ...
    train_model(dataset_root, model_save_dir, model=model, dataset_split=0.2, num_classes=num_model_classes, dataset_test_root=dataset_test_root, num_epochs=num_epochs, dataset_class_params=class_params)
