...
def get_resnet50_model_instance_segmentation(num_classes, model_load_dir=None):
    ...
    model.roi_heads.mask_predictor = MaskRCNNPredictor(in_features_mask,
                                                       hidden_layer,
                                                       num_classes)
    # Set nms threshold for the RoI heads to 0.3
    model.roi_heads.nms_thresh = 0.07

    # Set score threshold for the RoI heads to 0.01
...
def test_model(dataset_root, model, num_classes, device, batch_size=2):
    ...
    dataset_test = WormDataset(dataset_root, get_transform(train=False), num_classes=num_classes,
...
def predict_with_model(imgs, model, device):
...
def draw_masks_on_imgs(imgs, predictions, score_thresh, pix_conf_thresh=0.5, label_type='egg', display_labels=False,
                       orig_file_paths=[], include_orig_img=True):
    ...
    for idx, prediction in enumerate(predictions):
        ...
        num_masks = prediction['masks'].shape[
            0]  # The shape of masks is (N, 1, H, W) where N is the number of masks for the given image
        # print(f'shape of masks: {prediction["masks"].shape[0]}')
        H = prediction['masks'].shape[2]
        W = prediction['masks'].shape[3]
        mask_merge_bw = np.zeros((H, W), dtype=bool)
        num_validmasks = 0
        for n in range(num_masks):
            ...
            if prediction['scores'][n] <= score_thresh:
                continue
            num_validmasks = num_validmasks + 1
            ...
            this_mask_bw = this_mask > pix_conf_thresh
            mask_merge_bw = np.logical_or(mask_merge_bw, this_mask_bw)
            this_mask = this_mask_bw.astype(np.uint8)
            this_mask = this_mask * 255
            # mask_pixel_area = cv2.countNonZero(this_mask)
            # area_this_mask = mask_pixel_area
            # area_total = area_total + area_this_mask
            ...
            cv_boxes_and_masks_img = cv2.addWeighted(cv_boxes_and_masks_img, 1, this_mask, 0.5,
                                                     0.0)  # Draws the transparent mask on top of the image
            ...
            cv_boxes_and_masks_img = cv2.rectangle(cv_boxes_and_masks_img, top_left_of_box, bottom_right_of_box, color,
                                                   2)
            ...
            if display_labels:
                ...
                cv_boxes_and_masks_img = cv2.putText(cv_boxes_and_masks_img, label_text,
                                                     (top_left_of_box[0], top_left_of_box[1] - 5),
                                                     cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)

        if orig_file_paths:
            cv_boxes_and_masks_img_orig = cv2.putText(cv_boxes_and_masks_img_orig, orig_file_paths[idx], (5, 15),
        ...
        mask_merge = mask_merge_bw.astype(np.uint8)
        mask_merge = mask_merge * 255
        area_total = cv2.countNonZero(mask_merge)
        images_with_masks.append(cv_boxes_and_masks_img)
        # area_total = area_total.item()
        total_areas.append(area_total)
        total_masknums.append(num_validmasks)

    # for masked_img in images_with_masks:
    #    cv2.imshow("masked image", masked_img)
    #    cv2.waitKey(0)
    return images_with_masks, total_areas, total_masknums
...
def gather_stage_length_data(dataset_root, model_dir, num_dataset_classes, num_model_classes):
    ...
    class_params = U.get_labeling_params()
    class_params = {x: class_params[x] for x in class_params if
                    class_params[x][0] not in exclude}  # Filter out the unwanted labels
    dataset = WormDataset(dataset_root, get_transform(train=False), num_classes=num_dataset_classes,
                          class_parameters=class_params)
    ...
    for image_idx in range(dataset.__len__()):
        ...
        if bbox_xmin < edge_pixels_thresh or bbox_ymin < edge_pixels_thresh or bbox_xmax > mask_shape[
            1] - edge_pixels_thresh or bbox_ymax > mask_shape[0] - edge_pixels_thresh:
...
def predict_images_in_folders(model, device, score_thresh, label_type, fd_path):
    ...
    if fd_path is None:
        root = tk.Tk()
        root.withdraw()
        pimages = r'C:\Users\fffei\Dropbox\Dian Hongfei shared\worm analysis\Matlab code\DC_CountingEggs'
        pfinal = r'C:\Users\fffei\Dropbox\Dian Hongfei shared\worm analysis\Matlab code\DC_CountingEggs'

        pimages = tk.filedialog.askdirectory(parent=root, initialdir=pimages,
                                             title='Please select the folder containing the images for inference.')
    ...
    for idx, folder_path in enumerate(folder_paths):
        ...
        for idx, img_path in enumerate(img_paths):
            ...
            N = 3
            # Upsample image using bilinear interpolation (order=1)
            img_np_upsampled = zoom(img_np, (N, N, 1), order=1)
            # Convert the upsampled numpy arrays back to PIL Images
            img = img_np_upsampled
            ...
            if idx % 10 == 0 or idx == len(img_paths) - 1:
                ...
                # print_and_flush("aa")
                masked_imgs, total_areas, total_masknums = draw_masks_on_imgs(images, prediction,
                                                                              score_thresh=score_thresh,
                                                                              label_type=label_type,
                                                                              display_labels=True,
                                                                              orig_file_paths=orig_file_paths)
                ...
                TOTAL_AREAS.extend(total_areas)
                TOTAL_MASKNUMS.extend(total_masknums)
                images = []
                orig_file_paths = []
        total_area_file_path = os.path.join(folder_path, 'TOTAL_AREAS.mat')
        total_masknums_file_path = os.path.join(folder_path, 'TOTAL_MASKNUMS.mat')
        Total_area_median = np.mean(TOTAL_AREAS)
        Total_masknums_median = np.mean(TOTAL_MASKNUMS)
        if not Total_area_median or Total_area_median is None:
            Total_area_median = 0
        if not Total_masknums_median or Total_masknums_median is None:
            Total_masknums_median = 0
        Total_Area_All.append(Total_area_median)
        Total_MaskNum_All.append(Total_masknums_median)
        savemat(total_area_file_path, {'TOTAL_AREAS': TOTAL_AREAS})
        savemat(total_masknums_file_path, {'TOTAL_MASKNUMS': TOTAL_MASKNUMS})

    return Total_Area_All, Total_MaskNum_All
...
def run_main(mfolder_path=None):
    print_and_flush("Starting the Python module.")
    ...
    # dataset_root = r'C:\Users\fffei\Dropbox\Paper\Egg-laying\Data\WormTrainingData'

    # Set up filepaths to save checkpoints after every epoch
    # model_save_dir = r'C:\Users\fffei\Dropbox\Paper\Egg-laying\Data\WormTestingData'
    ...
    model_dir = r'D:\C elegans Team Dropbox\HONGFEI JI\Paper\Egg-laying\Data\WormTrainingData_2023July\TrainedModel\model_2023-08-01_08-31-05_resnet50_2classes_25epochs_24Done.pth'  # Gender Model trained full Combined_ADF-YF-ZL_WholeHeadMasks_Augmented dataset


    # model = get_resnet50_model_instance_segmentation(num_model_classes) # Start from scratch
    model = get_resnet50_model_instance_segmentation(num_model_classes,
                                                     model_load_dir=model_dir)  # Continue training a previous model
    ...
    # image_dir = r'C:\Users\fffei\Dropbox\Paper\Egg-laying\Data\WormTestingData\DatasetImages\_R1_C1_W1_F1dop3_20fps0040.png'
    ...
    Total_Area_All, Total_MaskNum_All = predict_images_in_folders(model, device, score_thresh, label_type, fd_path=mfolder_path)
    ...
    # print('\n\n======================================================================\n\n')
    print_and_flush("Finish the Python module.")
    return Total_Area_All, Total_MaskNum_All


if __name__ == "__main__":
    Total_Area_All, Total_MaskNum_All = run_main()
