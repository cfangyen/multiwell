...
utils_path = os.path.dirname(sys.path[0]) # Get the current path to the script and remove the mask_rcnn directory
...
base_path = utils_path # Create the path to the base folder
# utils_path.append('mask_rcnn_utils') # append the utils folder to the path
utils_path = os.path.join(utils_path, 'mask_rcnn_utils')
...
class WormDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # print(f"Processing data at index: {self.imgs[idx]}")  # Add this line
            ...
            img_np = np.array(img)
            mask_np = np.array(mask) # RGB shape (512, 612, 3)
            ...
            # Upsample mask using nearest neighbor (order=0)
            mask_np_upsampled = zoom(mask_np, (N, N, 1), order=0)

            # Convert the upsampled numpy arrays back to PIL Images
            img = img_np_upsampled
            mask = mask_np_upsampled

            # # Display the upsampled image and mask
            ...
            target["filename"] = self.imgs[idx]  # add this line
            ...
            print(f"Error processing image: {self.imgs[idx]}")
            raise e
