...
def get_resnet50_model_instance_segmentation(num_classes, model_load_dir=None):
    ...
    if model_load_dir is not None:
        model.load_state_dict(torch.load(model_load_dir, map_location=torch.device('cpu')))
...
def predict_images_in_folders(model, device, score_thresh, label_type, fd_path):
    ...
    if fd_path is None:
        root = tk.Tk()
        root.withdraw()
        pimages = r'D:\Dropbox\Paper\Egg-laying\Data\Test\Images for egg counting'
        pfinal = r'D:\Dropbox\Paper\Egg-laying\Data\Test\Images for egg counting'
    ...
    for idx, folder_path in enumerate(folder_paths):
        ...
        Total_area_median = np.median(TOTAL_AREAS)
        Total_masknums_median = np.median(TOTAL_MASKNUMS)
        Total_Area_All = Total_Area_All.extend(Total_area_median)
        Total_MaskNum_All = Total_MaskNum_All.extend(Total_masknums_median)
...
def run_main(mfolder_path=None):
    ...
    model_dir = r'D:\Dropbox\Paper\Egg-laying\Data\WormTrainingData_2023July\TrainedModel\model_2023-08-01_08-31-05_resnet50_2classes_25epochs_24Done.pth'  # Gender Model trained full Combined_ADF-YF-ZL_WholeHeadMasks_Augmented dataset
